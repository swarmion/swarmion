...
npx create-swarmion-app my-project-name
```
...
A **Service** represent a backend serverless service using the [Serverless framework](https://serverless.com/framework/). When the command `pnpm deploy` is run inside, it will deploy a stack to AWS.
...
Here you can see that the `backend-core` service has the `core-contract` and `serverless-configuration` libraries as dependencies.
