...
```bash
yarn nx generate @swarmion/nx-plugin:library
```
...
For the rest of the tutorial, we will consider that the `yarn watch` command is running, hence that all changes to the library are automatically packaged.
...
```bash
cd backend/users
yarn add @my-project-name/utils
```
...
```bash
yarn serverless invoke local --function health
