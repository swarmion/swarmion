...
:::info
The `implicit` dependency between `users` and `core` denotes a deployment dependency. Since the `users` service is using a ressource of `core` (the API Gateway), we must ensure that the `core` service is deployed before the `users` service.
...
This command will leverage the dependency graph of the project and deploy all the services in the right order. This means that `core` will be deployed, then `users`.
